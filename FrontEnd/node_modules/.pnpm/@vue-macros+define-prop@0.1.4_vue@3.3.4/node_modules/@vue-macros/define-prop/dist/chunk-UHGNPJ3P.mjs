...
// ../api/src/vue/emits.ts
import {
  babelParse as babelParse3,
  isStaticExpression as isStaticExpression2,
...
import { DEFINE_PROP as DEFINE_PROP2 } from "@vue-macros/common";
...
import {
  ...
  DEFINE_PROPS as DEFINE_PROPS2,
  HELPER_PREFIX,
  MagicString as MagicString2,
  getTransformResult,
  importHelperFn,
  isCallOf as isCallOf2,
  parseSFC as parseSFC2,
  walkAST
} from "@vue-macros/common";
import { inferRuntimeType as inferRuntimeType2, resolveTSReferencedType as resolveTSReferencedType2 } from "@vue-macros/api";
...
async function transformDefineProp(code, id, edition = "kevinEdition", isProduction = false) {
  if (!code.includes(DEFINE_PROP3))
    return;
  const { scriptSetup, getSetupAst } = parseSFC2(code, id);
  if (!scriptSetup)
    return;
  ...
  walkAST(setupAst, {
    enter(node, parent) {
      if (isCallOf2(node, DEFINE_PROP3)) {
        hasDefineProp = true;
      ...
      } else if (isCallOf2(node, DEFINE_PROPS2)) {
        hasDefineProps = true;
      }
    }
  });
  if (hasDefineProps && hasDefineProp)
    throw new Error(
      `${DEFINE_PROP3} can not be used in the same file as ${DEFINE_PROPS2}.`
    );
...
`
  ...
  async function resolveTSType(type) {
    const resolved = await resolveTSReferencedType2({
      scope: {
        filePath: id,
    ...
    return resolved && inferRuntimeType2(resolved);
  }
}
